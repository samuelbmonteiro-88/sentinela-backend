...
app.use(cors({
  origin: (origin, cb) => {
    // Permite: sem origin (SW, mobile), origens conhecidas
    if (!origin || origin === 'null' || origensPermitidas.includes(origin)) return cb(null, true);
...
app.post('/modo', async (req, res) => {
  ...
  await pool.query(`
    INSERT INTO modo_trabalho (device_id, modo, atualizado_em)
    VALUES ($1, $2, NOW())
...
app.get('/modo/:deviceId', async (req, res) => {
  const { rows } = await pool.query(
    'SELECT modo FROM modo_trabalho WHERE device_id = $1', [req.params.deviceId]
  );
...
// ── EXPORTAR DADOS (CSV) ─────────────────────────────────────────────────────
app.get('/exportar/:deviceId', async (req, res) => {
  const { deviceId } = req.params;
  const { rows } = await pool.query(`
    SELECT estado, score, sono, carga, decisao, pavio, fisica, emocional,
           bebi_agua, alonguei, fez_pausa, criado_em
    FROM checkins
    WHERE device_id = $1
    ORDER BY criado_em ASC
  `, [deviceId]);

  const header = 'estado,score,sono,carga,decisao,pavio,fisica,emocional,bebi_agua,alonguei,fez_pausa,criado_em';
  const lines = rows.map(r =>
    [r.estado, r.score, r.sono, r.carga, r.decisao, r.pavio, r.fisica, r.emocional,
     r.bebi_agua, r.alonguei, r.fez_pausa, r.criado_em].join(',')
  );
  const csv = [header, ...lines].join('\n');

  res.setHeader('Content-Type', 'text/csv');
  res.setHeader('Content-Disposition', 'attachment; filename="sentinela.csv"');
  res.send(csv);
});
